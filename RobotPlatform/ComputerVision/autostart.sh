...
if !([ -d "$INSTALL_FOLDER" ]); then
    cd /python3_ws \
        && catkin clean --y \
        && catkin init \
        && catkin config --install -DPYTHON_EXECUTABLE=/usr/bin/python3.6 -DPYTHON_INCLUDE_DIR=/usr/include/python3.6m -DPYTHON_LIBRARY=/usr/lib/aarch64-linux-gnu/libpython3.6m.so -DBOOST_ROOT=/usr/include/boost/
