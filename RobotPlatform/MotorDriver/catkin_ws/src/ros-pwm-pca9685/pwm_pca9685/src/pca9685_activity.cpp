...
PCA9685Activity::PCA9685Activity(ros::NodeHandle &_nh, ros::NodeHandle &_nh_priv) :
  nh(_nh), nh_priv(_nh_priv) {
    ...
    // if(!param_timeout.empty())
    // {
    //     std::copy(param_timeout.begin(), param_timeout.end()-1,
    //         std::ostream_iterator<int>(vts, ", "));
    //     vts << param_timeout.back();
    // }
    // ROS_ERROR("TIMEOUT %s", vts.str().c_str());
...
bool PCA9685Activity::start() {
    ...
    if(!sub_command) sub_command = nh.subscribe("pca9685/motor_command", 1, &PCA9685Activity::onCommand, this);
